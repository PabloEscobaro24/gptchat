...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Привет! Я <b>ChatGPT</b> бот реализован с помощью GPT-4 Openair API🤖\n\n"
    ...
    async with user_semaphores[user_id]:
        ...
        try:
            ...
            async for gen_item in gen:
                ...
                if status == "not_finished":
                    ...
                    raise ValueError(f"Статус потоковой передачи {status} неизвестен")
                ...
                if i == 0:  # send first message (then it'll be edited if message streaming is enabled)
                    try:                    
                        ...
                        if str(e).startswith("Сообщение не может быть пустым"):  # first answer chunk from openai was empty
        ...
        if n_first_dialog_messages_removed > 0:
            if n_first_dialog_messages_removed == 1:
                text = "✍️ <i>Note:</i> Ваш текущий диалог слишком длинный, так что ваш <b>first message</b> был вырван из контекста.\n Отправь коменду /new для старта нового диалога"
            else:
                text = f"✍️ <i>Note:</i> Ваш текущий диалог слишком длинный, так <b>{n_first_dialog_messages_removed} первые сообщения</b> были вырваны из контекста.\n Отправь команду /new для старта нового диалога"
...
async def is_previous_message_not_answered_yet(update: Update, context: CallbackContext):
    ...
    if user_semaphores[user_id].locked():
        text = "⏳ Пожалуйта <b>дождитесь</b> ответа на предыдущее сообщение"
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("/mode", "Выберите режим чата"),
